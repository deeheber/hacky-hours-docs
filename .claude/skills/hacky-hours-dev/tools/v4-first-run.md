...
```
~/.hacky-hours/
├── version                 ← installed framework version (file contents: "4.0.0-dev")
...
Use `mkdir -p` for safety. Write `version` with the literal text `4.0.0-dev` (no trailing newline beyond one).
